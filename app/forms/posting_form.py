...
class PostingForm(FlaskForm):
    address = StringField('address', validators=[DataRequired(), Length(min=5, max=100, message='Address must be 5-100 characters.')])
    city = StringField('city', validators=[DataRequired(), Length(min=3, max=100, message='City must be 3-100 characters.')])
